...
class MXJSWidgetHelper {
      ...
      if (newValue && typeof newValue === 'object') {
        ...
        if (objectSymbols.length > 0) {
          ...
          for (var i = 0; i < len; ++i) {
            ...
            if (typeof k === 'symbol') {
              replacement[key] = this.convertSymbolToStringKey(value);
            } else {
  ...
  // 将所有Symbol都转换成String类型
  static convertSymbolToStringKey(object) {
    ...
    // 含Symbol处理下，遍历所有Symbol转成String
    var map = {};
    var len = objectSymbols.length;
    for (var i = 0; i < len; ++i) {
      var key = objectSymbols[i].description;
      var value = object[objectSymbols[i]];
      if (typeof k === 'symbol') {
        map[key] = this.convertSymbolToStringKey(value);
      } else {
        map[key] = value;
      }
    }
