package com.mojitox.mxflutter.framework.system;
...
public class SafelyLibraryLoader {
    ...
    //视频so load失败
    public static final int VIDEO_SYS_EXCEPTION = 6;

    @IntDef({SYS_ERROR, SYS_EXCEPTION, RECOVER_FAIL, RECOVER_UNPACK_LOAD_SUC, RECOVER_LOAD_SUC, VIDEO_SYS_EXCEPTION})
    ...
    /**
     * 全新安装或者升级安装先清掉这个目录，防止so版本升级后仍然读取到旧的so /data/data/com.tencent.reading/files/libso
     *
     * @param context
     * @return
     */
    private synchronized static File getLibraryFolder(Context context) {
    ...
    /**
     * /data/data/com.tencent.reading/files/libso/libshadow.so 根据libname获取lib路径
     *
     * @param context
     * @param str
     * @return
    ...
    /**
     * 解压apk包lib so 到 libso目录下面 libso路径 /data/data/com.tencent.reading/files/libso/libshadow.so
     *
     * @param context
     * @param libname
     * @param file
